...
(function () {
  function RgbQuant(opts) {
    ...
    this.hueStats = this.minHueCols
      ? new HueStats(this.hueGroups, this.minHueCols)
      : null;
    ...
    this.colorDist =
      opts.colorDist == "manhattan" ? distManhattan : distEuclidean;
  ...
  RgbQuant.prototype.reduce = function reduce(
    img,
    retType,
    dithKern,
    dithSerp
  ...
  RgbQuant.prototype.dither = function (img, kernel, serpentine) {
    ...
    for (var y = 0; y < height; y++) {
      ...
      for (
        var x = dir == 1 ? 0 : width - 1, xend = dir == 1 ? width : 0;
        x !== xend;
        x += dir
      ) {
        // Image pixel
        ...
        for (
          var i = dir == 1 ? 0 : ds.length - 1, end = dir == 1 ? ds.length : 0;
          i !== end;
          i += dir
        ) {
          var x1 = ds[i][1] * dir,
  ...
  RgbQuant.prototype.palette = function palette(tuples, noSort) {
    ...
    return tuples
      ? this.idxrgb
      : new Uint8Array(new Uint32Array(this.idxi32).buffer);
  };
    ...
    this.idxi32.sort(function (a, b) {
      ...
      var hueA =
        rgbA[0] == rgbA[1] && rgbA[1] == rgbA[2]
          ? -1
          : hueGroup(hslA.h, self.hueGroups);
      var hueB =
        rgbB[0] == rgbB[1] && rgbB[1] == rgbB[2]
          ? -1
          : hueGroup(hslB.h, self.hueGroups);
  ...
  RgbQuant.prototype.cacheHistogram = function cacheHistogram(idxi32) {
    for (
      var i = 0, i32 = idxi32[i];
      i < idxi32.length && this.histogram[i32] >= this.cacheFreq;
      i32 = idxi32[i++]
    )
